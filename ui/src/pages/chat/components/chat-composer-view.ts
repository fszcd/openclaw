...
import { renderChatComposerSurface } from "../../../components/chat-composer-surface.ts";
...
export function renderChatComposerView(context: ChatComposerViewContext) {
  ...
  return renderChatComposerSurface({
    questionComposer: compoundQuestionComposer,
    overlay: html`${props.anchoredNotices ?? nothing} ${composerAlerts} ${fallbackStatus}
    ${
    ...
    ${queue} ${goalCard}`,
    afterInput: html`${
      !showComposerInput && props.disabledBanner?.kind === "composer-replacement"
        ? disabledBanner
        : nothing
    }${composerUnderlaps}`,
    input: showComposerInput
      ? {
          offline: props.offline,
          dictating: dictation?.active,
          busy: props.disabledReasonBusy,
          inputRef: state.composerInputRef ?? undefined,
          events: {
            show: handleChatComposerDropdownShow,
            afterShow: restorePointerOpenedChatComposerTrigger,
            dismissInvocations: () => {
          ...
          ${mentionMenuVisible ? state.mentionMenu.render(mentionMenuHost, requestUpdate) : nothing}`,
          lede: html`${goalComposer.render()} ${renderAttachmentPreview(props)}
          ...
          editor: html`<textarea
            ...
            >`,
          lead: html`${renderChatComposerPlusMenu({
            attachments: props,
